...
function perplexityProvider(apiKey: string, model: string): AiProvider {
  return {
    ...
    async complete(params) {
      ...
      if (!response.ok) throw new AiError(`Perplexity request failed (HTTP ${response.status}).`);
      
      const body = (await response.json()) as { choices?: Array<{ message?: { content?: string } }> };
      const text = body.choices?.[0]?.message?.content;
      if (typeof text !== 'string' || text.trim().length === 0) throw new AiError('Perplexity returned no message.');
      return { text };
    },
  };
}
...
function openaiProvider(apiKey: string, model: string): AiProvider {
  return {
    ...
    async complete(params) {
      ...
      const body = (await response.json()) as { choices?: Array<{ message?: { content?: string } }> };
      const text = body.choices?.[0]?.message?.content;
      if (typeof text !== 'string' || text.trim().length === 0) throw new AiError('OpenAI returned no message.');
      return { text };
    },
  };
}
...
function anthropicProvider(apiKey: string, model: string): AiProvider {
  return {
    ...
    async complete(params) {
      ...
      if (typeof text !== 'string' || text.trim().length === 0) throw new AiError('Anthropic returned no message.');
      return { text };
    },
  };
}

// ✅ C'EST ICI QUE TOUT SE JOUE : Le type doit accepter les 3 fournisseurs
