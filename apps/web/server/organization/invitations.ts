...
import { invitations, memberships, users } from '@kloyya/db/schema';
...
export async function createInvitation(
  ...
  return db.transaction(async (tx) => {
    ...
    // no-op. Email is denormalized onto the profile, so the roster is checked
    // through `users` — no separate identity table to join.
    const [existingMember] = await tx
      .select({ id: memberships.id })
      .from(memberships)
      .innerJoin(users, eq(users.id, memberships.userId))
      .where(and(eq(memberships.workspaceId, inviter.workspaceId), eq(users.email, email)))
      .limit(1);
