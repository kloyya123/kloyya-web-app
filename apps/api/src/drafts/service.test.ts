...
import { createDraft, deleteDraft, getDraft, listDrafts, updateDraft } from './service.js';
...
async function workspace(email: string): Promise<StartContext> {
  const { userId } = await signUp(app, { email, password: 'a sufficiently long passphrase', name: 'Writer' });
